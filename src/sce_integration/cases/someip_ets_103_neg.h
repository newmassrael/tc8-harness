...
#include "someip_ets_103_neg_sm.h"

namespace tc8::sce::cases {

using SomeipEts103NegSM = ::SCE::Generated::someip_ets_103_neg::someip_ets_103_neg;

}  // namespace tc8::sce::cases

namespace tc8::sce {

// Self-validation of §5.1.6 SOMEIP_ETS_103: GetLastValueOfEventTCP must return the cached
// event UInt8 (0x08). kEtsFaultFieldValueWrong makes the harness-owned EtsImpl getter return
// the cached value bit-flipped (0x08 ^ 0xFF = 0xF7 != 0x08), so a buggy DUT surfaces a
...
template <>
struct TestCaseTraits<cases::SomeipEts103NegSM>
    : SomeIpEtsFaultNegBase<cases::SomeipEts103NegSM> {
    static constexpr std::string_view kCaseId      = "SOMEIP_ETS_103_NEG";
    static constexpr std::string_view kDescription =
        "Self-validation of SOMEIP_ETS_103: the kEtsFaultFieldValueWrong app fault makes "
        "GetLastValueOfEventTCP return a value other than the cached 0x08; a conformant DUT "
        "returns 0x08";

    // Arm the getter value fault, then drive the same activate / subscribe / get-last chain
    // the positive uses. Only the final GetLastValueOfEventTCP readback flips (0xF7 != 0x08).
    static void stimulus(Captured& /*c*/,
        ...
        // GetLastValueOfEventTCP — Method 0x3B; the armed getter returns 0x08 ^ 0xFF = 0xF7.
...
TC8_REGISTER_CASE(::tc8::sce::cases::SomeipEts103NegSM, someip_ets_103_neg)
