#pragma once

#include <cstdint>
#include <string_view>

#include "tc8/upper_tester_protocol.h"

#include "sce_integration/case_registry.h"
#include "sce_integration/cases/_fault_flavor_arm.h"
#include "sce_integration/cases/_udp_traits_base.h"
#include "sce_integration/test_runner.h"
#include "sce_integration/udp_pilot_common.h"

#include "udp_fields_07_neg_sm.h"

namespace tc8::sce::cases {

using UdpFields07NegSM = ::SCE::Generated::udp_fields_07_neg::udp_fields_07_neg;

}  // namespace tc8::sce::cases

namespace tc8::sce {

template <>
struct TestCaseTraits<cases::UdpFields07NegSM>
    : UdpEgressFaultNegBase<cases::UdpFields07NegSM> {
    static constexpr std::string_view kCaseId      = "UDP_FIELDS_07_NEG";
    static constexpr std::string_view kDescription =
        "Self-validation of UDP_FIELDS_07: the lwIP kUdpFaultLengthWrong egress "
        "flavor rewrites the DUT zero-payload egress UDP Length; conformant emits 8";
    static void stimulus(Captured& /*c*/,
                         const ::tc8::TestConfig& cfg,
                         std::string_view iface) {
        emitEgressFlavorArm(cfg, iface, ::tc8::ut::kUdpFaultLengthWrong);
        ::tc8::sce::udp::emitTriggerSendUdp(
            cfg, iface, /*req_id=*/1,
            /*dut_src_port=*/20003,
            /*target_ip_be=*/cfg.ipv4.tester_ip,
            /*target_port=*/::tc8::sce::udp::kDataPort,
            /*payload=*/nullptr,
            /*payload_len=*/0,
            ::tc8::ut::kTesterSrcPort,
            cfg.dut.mac);
    }
};

}  // namespace tc8::sce

TC8_REGISTER_CASE(::tc8::sce::cases::UdpFields07NegSM, udp_fields_07_neg)
