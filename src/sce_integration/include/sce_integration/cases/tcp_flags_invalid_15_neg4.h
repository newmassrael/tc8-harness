...
#include "tcp_flags_invalid_15_neg4_sm.h"

namespace tc8::sce::cases {

using TcpFlagsInvalid15Neg4SM = ::SCE::Generated::tcp_flags_invalid_15_neg4::tcp_flags_invalid_15_neg4;

}  // namespace tc8::sce::cases

namespace tc8::sce {

// Self-validation of §4.8.6.6 TCP_FLAGS_INVALID_15 phase 4 (FIN-WAIT-2): a conformant DUT in
// FIN-WAIT-2 silently drops an out-of-window RST (RFC 793 §3.9). kTcpSynthRstOnDisruptive makes
...
struct TestCaseTraits<cases::TcpFlagsInvalid15Neg4SM>
    ...
    static constexpr std::string_view kDescription  =
        ...
        "RST in FIN-WAIT-2; a conformant DUT stays silent";

    // Mirrors the positive's phase-4 seam: DUT CLOSE + tester auto-ACK (no AckDrop) -> FIN-WAIT-2.
    static void stimulus(Captured& /*c*/,
        ...
        const std::uint16_t local_port  = kBasicsActiveLocalPort  + kTcpFlagsInvalid15Phase3LocalOffset;
        const std::uint16_t remote_port = kBasicsActiveRemotePort + kTcpFlagsInvalid15Phase3LocalOffset;

        auto open = driveSeamActiveOpen(dut, cfg, local_port, remote_port);
        const int tester_fd = open.listener.acceptOne();
        if (open.conn) dut.tcpControl()->closeTcp(open.conn->socket);
        std::this_thread::sleep_for(std::chrono::milliseconds(200));   // tester auto-ACK -> FW2
...
TC8_REGISTER_CASE(::tc8::sce::cases::TcpFlagsInvalid15Neg4SM, tcp_flags_invalid_15_neg4)
