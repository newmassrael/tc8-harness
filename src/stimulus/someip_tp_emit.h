...
// 16; default the spec-recommended 1392). Returns 0 when every segment was sent, or the
// first negative sendUdpUnicast sentinel on failure (remaining segments are not sent).
// Propagates the Segmenter's exceptions on a bad max_segment_payload or an over-long
// payload (see someiptp::Segmenter).
