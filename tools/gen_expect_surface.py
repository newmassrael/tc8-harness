#!/usr/bin/env python3
"""Generate the Bash + Rust mirrors of the base `--expect` identity surface from
the single source tools/expect_surface.def, so the two test drivers cannot drift
(docs/tech-debt.md TD-12).
...
SH_OUT = ROOT / "dut/env/expect_surface.gen.sh"
...
# --- Bash render ------------------------------------------------------------
def _bash_value(r: Row) -> str:
    if r.source in ("vs_id", "vs_sd"):
        return f"${{DUT_ID[{r.ref}]}}"
    if r.source == "wire":
        return f"${{TC8_WIRE_{r.ref}}}"
    if r.source == "wire_alias":
        return f"${{TC8_TOPOLOGY_{r.ref}:-$TC8_WIRE_{r.ref}}}"
    if r.source == "dut_ip":
        return "${DUT_IP4}"
...
def render_sh(buckets: list[Bucket], rows: list[Row]) -> str:
    lines = [
    ...
    for b in buckets:
        brows = [r for r in rows if r.bucket == b.id]
        lines.append(f"tc8_expect_{b.id}() {{")
        if b.kind == "runtime":
            lines.append("    local dut_mac=$1")
            printf = ["    printf '%s\\n' \\"]
            for i, r in enumerate(brows):
                cont = " \\" if i < len(brows) - 1 else ""
                printf.append(f'        --expect "{r.key}={_bash_value(r)}"{cont}')
            lines.extend(printf)
        else:
            lines.append(f"    {b.bash_array}=(")
            for r in brows:
                for d in r.docs:
                    lines.append(("        # " + d).rstrip())
                lines.append(f'        --expect "{r.key}={_bash_value(r)}"')
            lines.append("    )")
        lines.append("}")
        lines.append("")
    return "\n".join(lines)
...
def main() -> int:
    ...
    buckets, rows = parse()
    outputs = {SH_OUT: render_sh(buckets, rows), RS_OUT: render_rs(buckets, rows)}
