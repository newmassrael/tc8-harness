...
// The host-NIC topologies (External, SshRemote + the shared HostTester transport)
// and the lwIP-tap topology live in submodules; this file keeps the Topology trait,
// the per-worker netns plumbing, and the single-pc topology. LwipTap moved here from
// `fixtures/` — it is a first-class host-NIC topology, not a verification fixture.
mod host;
mod lwip_tap;
