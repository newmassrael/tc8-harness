...
- Compile `udp portrange 30490-30500 or tcp portrange 30490-30500` in-kernel
- Expose port range via harness_config.yaml (TC8 §4.6 UDP, §4.8 TCP)
...
- Enable first end-to-end happy path for sample Service A (TC8 SOMEIP_ETS_1)
```
...
- Cover deadline_exceeded via `<send delay="50ms"/>` timer race (plan §9)
- Add malformed TLV fixture for SOME/IP Server FORMAT assertion
- All three verdicts (pass / fail_timeout / fail_field) produce donedata
```
...
- Produce tc8-harness binary with cap_net_raw setcap hint in install step
```
...
- Encapsulate pcap_open_live / pcap_compile / pcap_setfilter in single RAII class
- Surface BPF compile errors with filter expression for ops diagnosis
- Replace ad-hoc capture loop in harness_main with managed worker thread
```
**개선점**:
1. 이모지 / 귀속 태그 없음
2. 3개 핵심 변경으로 응축
